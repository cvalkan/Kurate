    ...
    <div className="space-y-1.5">
      ...
      <div className="relative h-20 border border-border rounded bg-secondary/10" data-testid={`si-dist-${metric}`}>
        <svg viewBox="0 0 100 50" preserveAspectRatio="none" className="w-full h-full">
          ...
          {data.mean && (
            <line
              x1={((data.mean - 1) / 9) * 100}
              y1={0}
              x2={((data.mean - 1) / 9) * 100}
              y2={50}
              stroke={color}
              strokeWidth={0.5}
              strokeDasharray="2 1"
          ...
          {metrics.slice(0, -1).map((m1, i) => (
            <tr key={m1}>
              ...
              {metrics.slice(1).map((m2, j) => {
                ...
                const key = `${m1} vs ${m2}`;
                const rho = matrix[key];
                if (rho === undefined) return <td key={m2} className="p-1 text-center text-muted-foreground">-</td>;
                return (
                  <td key={m2} className={`p-1 text-center font-mono font-medium rounded ${cellColor(rho)}`}>
                    {rho.toFixed(2)}
                  </td>
                );
              })}
...
export function SiRatingSection({ category, modelCorrelationData }) {
  ...
  if (loading) return <div className="h-40 bg-secondary/20 rounded-lg animate-pulse" />;
  if (!data || data.status !== "ok") {
    return (
      <div className="mb-10 p-6 border border-border rounded-lg text-center text-muted-foreground">
        ...
        <p className="text-xs">Not enough SI rating data yet ({data?.total_papers || 0} papers rated).</p>
      </div>
    );
  }
    ...
    <div className="mb-10" data-testid="si-rating-section">
      <div className="mb-4 pb-2 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground">
          Distribution of AI single-item ratings across {data.total_papers} papers.
