...
import {
  Beaker, Play, Square, Loader2, AlertCircle, CheckCircle2, XCircle,
...
function RateBadge({ rate, label, sub, size = "normal" }) {
  ...
  return (
    <div className={`p-3 rounded-lg border border-border ${bg} text-center`}>
      ...
      <div className={`${textSize} font-semibold font-mono ${color}`}>{rate}%</div>
      {sub && <div className="text-[10px] text-muted-foreground">{sub}</div>}
    </div>
  );
}

function MAEBadge({ mae, label }) {
  const color = mae <= 0.8 ? "text-green-600" : mae <= 1.2 ? "text-amber-600" : "text-red-600";
  const bg = mae <= 0.8 ? "bg-green-50" : mae <= 1.2 ? "bg-amber-50" : "bg-red-50";
  return (
    <div className={`p-3 rounded-lg border border-border ${bg} text-center`}>
      <div className="text-[10px] text-muted-foreground mb-0.5">{label}</div>
      <div className={`text-xl font-semibold font-mono ${color}`}>{mae}</div>
...
export default function SciPostPage() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      if (s.data?.fetching || s.data?.running) {
        setIsStarting(false);
      }
  ...
  const fetchAndRun = async () => {
    ...
    try {
      ...
      if (res.data.status === "started") {
        toast.success(`Started! Fetching ${numPapers} papers from SciPost...`);
    ...
    <div className="container mx-auto px-4 md:px-6 max-w-5xl py-6 md:py-10">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground max-w-3xl">
          ...
          AI rates each paper on <strong>validity</strong>, <strong>significance</strong>, <strong>originality</strong>, and <strong>clarity</strong> — 
          then we compare against human referee scores.
        </p>
      </div>
        ...
        <div className="border border-border rounded-lg p-4 mb-6 bg-secondary/10 space-y-3" data-testid="admin-controls">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <Input
                type="number" min={5} max={100} value={numPapers}
                onChange={e => setNumPapers(parseInt(e.target.value) || 15)}
                className="w-20 h-8 text-xs" data-testid="num-papers-input"
                disabled={running}
              />
            </div>
            {!running ? (
        ...
        <div className="space-y-5">
          ...
          <div className="border border-border rounded-lg p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              "Close" = AI rating within ±1 of human (on 1-6 scale). MAE = Mean Absolute Error.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {DIMENSIONS.map(dim => {
                ...
                if (!d) return null;
                return (
                  <div key={dim} className={`p-3 rounded-lg border ${DIM_COLORS[dim]} border-border`}>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      {DIMENSIONS.map(d => (
                        <th key={d} className="text-center py-2 px-2 capitalize">{d}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(results.by_model).map(([mk, dims]) => (
                      <tr key={mk} className="border-b border-border/30">
                        ...
                        {DIMENSIONS.map(dim => {
                          ...
                          return (
                            <td key={dim} className={`text-center font-mono ${color}`}>
                              {s.close_rate}%
                            </td>
                          );
                        })}
          ...
          {results.rating_distribution && (
            <div className="border border-border rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-xs text-muted-foreground mb-2">Human Ratings</div>
                  <div className="flex gap-1">
                    {[1,2,3,4,5,6].map(r => {
                      const count = results.rating_distribution.human?.[r] || 0;
                      const max = Math.max(...Object.values(results.rating_distribution.human || {}), 1);
                ...
                <div>
                  <div className="text-xs text-muted-foreground mb-2">AI Ratings</div>
                  <div className="flex gap-1">
                    {[1,2,3,4,5,6].map(r => {
                      const count = results.rating_distribution.ai?.[r] || 0;
                      const max = Math.max(...Object.values(results.rating_distribution.ai || {}), 1);
                      const height = Math.max((count / max) * 60, 4);
                      return (
                        <div key={r} className="flex flex-col items-center">
                          <div className="text-[10px] text-muted-foreground mb-1">{count}</div>
              ...
              <div className="text-[10px] text-muted-foreground mt-2">
                Scale: 1=Poor, 2=Low, 3=OK, 4=Good, 5=High, 6=Top
              </div>
            </div>
          )}
                    ...
                    {results.samples.map((s, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-border/20 hover:bg-secondary/10">
                          <td className="px-2 py-1 max-w-[200px] truncate" title={s.paper_title}>
                            {s.paper_title}
                          </td>
                          <td className={`text-center px-1.5 py-1 capitalize ${DIM_COLORS[s.dimension]?.split(' ')[0] || ''}`}>
                          ...
                          <td className={`text-center px-1.5 py-1 font-mono ${match ? 'text-green-600' : 'text-red-500'}`}>
                            {s.ai_consensus?.toFixed(1) || '—'}
                          </td>
                          {Object.entries(s.ai_ratings || {}).slice(0, 3).map(([mk, rating]) => (
                            <td key={mk} className="text-center px-1.5 py-1 font-mono text-muted-foreground">
                              {rating || '—'}
                            </td>
                          ))}
                        </tr>
