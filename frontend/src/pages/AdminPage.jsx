...
import {
  Settings, Activity, LogOut, FileText, Save, HelpCircle, FlaskConical, MessageSquare, Users, Trophy,
  Pause, Play,
} from "lucide-react";
...
function AdminTournaments() {
  ...
  return (
    <div className="space-y-4" data-testid="admin-tournaments">
      <div className="flex items-center justify-between">
        <h2 className="font-heading text-lg font-medium">Tournament Registry</h2>
        <span className="text-xs text-muted-foreground">{tournaments.length} tournaments</span>
      </div>

      {tournaments.length === 0 ? (
        <div className="p-8 text-center text-muted-foreground border border-border rounded-lg">
          <Trophy className="h-8 w-8 mx-auto mb-2 opacity-30" />
          <p className="text-sm">No tournaments registered. They will be created on next restart.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {tournaments.map(t => (
            <div key={t.tournament_id} className={`p-4 border rounded-lg ${t.status === "paused" ? "border-border/50 bg-secondary/10 opacity-60" : "border-border"}`} data-testid={`tournament-${t.tournament_id}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-secondary text-muted-foreground">{t.mode}</span>
                  {t.stats?.goals_met && (
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={`h-7 text-xs gap-1 ${t.status === "active" ? "text-amber-600 hover:text-amber-700" : "text-green-600 hover:text-green-700"}`}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                <div className="text-muted-foreground">
                  Papers: <span className="font-mono text-foreground">{t.stats?.papers || 0}</span>
                </div>
                <div className="text-muted-foreground">
                  Matches: <span className="font-mono text-foreground">{t.stats?.matches || 0}</span>
                </div>
                <div className="text-muted-foreground">
                  Min matches: <span className="font-mono text-foreground">{t.goals?.min_matches || "?"}</span>
                </div>
                <div className="text-muted-foreground">
                  CI target: <span className="font-mono text-foreground">{"\u00B1"}{t.goals?.ci_target || "?"}%</span>
                </div>
              </div>
              {t.updated_at && (
      ...
      <div className="p-3 bg-secondary/30 rounded-lg text-xs text-muted-foreground">
        Tournaments are automatically created for each active primary category. Pausing a tournament stops the scheduler from running matches for that category. Tournament goals and stats are updated each scheduler cycle.
      </div>
    </div>
  );
}
