...
export default function ValidationHubPage() {
  ...
  const sectionMeta = {
    ...
    "exp-summarizer-ab": { title: "Summarizer A/B", desc: "How does the choice of summarizer model (GPT-5.2, Gemini 3 Pro, Opus 4.5/4.6) affect tournament accuracy? Same-pair comparison across all summarizers." },
        ...
        <nav className="w-56 shrink-0 space-y-3 max-h-[calc(100vh-120px)] overflow-y-auto" data-testid="validation-sidebar">
          ...
          {isAdmin && (
            <CollapsibleGroup label="Experiments" icon={FlaskRound} defaultOpen={selected?.startsWith("exp-")}>
              ...
              <CollapsibleGroup label="Summarizer A/B" defaultOpen={selected === "exp-summarizer-ab"}>
                <NavItem item={{ id: "exp-summarizer-ab", label: "GPT vs Gemini vs Opus", sub: "Summary model comparison" }} selected={selected} onSelect={setSelected} />
              </CollapsibleGroup>
