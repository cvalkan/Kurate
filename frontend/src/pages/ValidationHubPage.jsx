...
import SIBenchmarkSection from "./SIBenchmarkSection";
...
export default function ValidationHubPage() {
  ...
  const meta = useMemo(() => {
    const STATIC_META = {
      ...
      "exp-human-ai-benchmark": { title: "Human vs AI Benchmark", desc: "How does AI judge agreement compare with inter-human agreement? Controlled same-pair comparison across all datasets with human evaluations. Includes Thurstonian ceiling analysis, Cohen's kappa, and stratification by difficulty." },
      "exp-si-benchmark": { title: "Human vs AI Benchmark (Single-Item)", desc: "Same methodology as the pairwise benchmark, but using AI single-item scores (Opus 4.6 Thinking, 1-10) instead of pairwise judgments. Both humans and AI produce scores — pairwise preferences derived from both." },
        ...
        <nav className="w-56 shrink-0 space-y-3 max-h-[calc(100vh-120px)] overflow-y-auto" data-testid="validation-sidebar">
          ...
          {/* Experiments */}
            ...
            <CollapsibleGroup label="Experiments" icon={FlaskRound} defaultOpen={selected?.startsWith("exp-")}>
              ...
              <CollapsibleGroup label="Judge Quality" defaultOpen={selected === "exp-judge-comparison" || selected === "exp-assessor-evaluator" || selected === "exp-human-ai-benchmark" || selected === "exp-si-benchmark"}>
                <NavItem item={{ id: "exp-human-ai-benchmark", label: "Human vs AI Benchmark", sub: "Pairwise concordance" }} selected={selected} onSelect={setSelected} />
                <NavItem item={{ id: "exp-si-benchmark", label: "Human vs AI (Single-Item)", sub: "Score-derived concordance" }} selected={selected} onSelect={setSelected} />
