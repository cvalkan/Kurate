...
export default function ValidationHubPage() {
  ...
  const meta = useMemo(() => {
    const STATIC_META = {
      ...
      "exp-human-ai-benchmark": { title: "Human vs AI Benchmark", desc: "How does AI judge agreement compare with inter-human agreement? Controlled same-pair comparison across all datasets with human evaluations, using the NeurIPS 2014 experiment as reference. Includes Thurstonian ceiling analysis, Cohen's kappa, and stratification by difficulty." },
        ...
        <nav className="w-56 shrink-0 space-y-3 max-h-[calc(100vh-120px)] overflow-y-auto" data-testid="validation-sidebar">
          ...
          {/* Experiments */}
            ...
            <CollapsibleGroup label="Experiments" icon={FlaskRound} defaultOpen={selected?.startsWith("exp-")}>
              ...
              <CollapsibleGroup label="Judge Quality" defaultOpen={selected === "exp-judge-comparison" || selected === "exp-assessor-evaluator" || selected === "exp-human-ai-benchmark"}>
                <NavItem item={{ id: "exp-human-ai-benchmark", label: "Human vs AI Benchmark", sub: "NeurIPS comparison" }} selected={selected} onSelect={setSelected} />
