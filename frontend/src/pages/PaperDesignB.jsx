                    ...
                    {validMatches.length === 0 ? (
                      ...
                      <>
                        {visibleMatches.map(m => (
                          <div key={m.id} className="px-4 sm:px-5 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50/70 transition-colors">
                            ...
                            {m.reasoning && <p className="text-xs text-slate-500 leading-relaxed mt-1.5">{m.reasoning}</p>}
                            <div className="flex items-center gap-2 mt-1.5 text-[11px] text-slate-400">
                              <span>{typeof m.model_used === "object" ? m.model_used.model : (m.model_used || "")}</span>
                              {m.created_at && <><span>·</span><span>{new Date(m.created_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</span></>}
                            </div>
                          </div>
                        ))}
