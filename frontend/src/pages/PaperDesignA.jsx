            ...
            <div className="inline-flex items-center rounded-sm border border-slate-200 p-0.5" role="tablist" aria-label="Paper versions">
              {[...paper.sibling_versions].sort((a, b) => (a.version || 0) - (b.version || 0)).map(v => {
                ...
                return isCurrent ? (
                  ...
                  <Link key={v.paper_id || v.version} to={`/paper/${v.paper_id}`} className="inline-flex items-center px-2.5 py-1 rounded-sm text-[11px] font-mono text-slate-500 hover:text-slate-900 hover:bg-slate-50 transition-colors" title={title}>{label}</Link>
                );
              })}
            </div>
          </div>
        ...
        {paper.is_latest_version === false && (() => {
          ...
          return (
            <div className="mb-6 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-sm flex items-center justify-between gap-3 text-xs text-amber-900">
              ...
              {latest?.paper_id && (
                <Link to={`/paper/${latest.paper_id}`} className="shrink-0 font-semibold text-amber-800 underline hover:no-underline">
