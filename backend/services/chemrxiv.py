...
async def fetch_chemrxiv_papers(category: str = "chemrxiv.IC", max_results: int = 50) -> List[Dict]:
    ...
    Flow:
    1. Read DOIs from seed file (generated by web scraping)
    2. For each DOI, call Cambridge DOI API to get full metadata + PDF URL
    3. Return enriched paper dicts ready for the tournament pipeline
    """
    ...
    async with httpx.AsyncClient(timeout=30.0, headers=HEADERS) as client:
        for seed in seed_papers[:max_results]:
            ...
            # Try to enrich via Cambridge DOI API
            enriched = await _enrich_via_api(client, doi, seed, category)
            if enriched:
                papers.append(enriched)
            else:
                # Fall back to seed data (no PDF but still useful)
                papers.append(seed)
