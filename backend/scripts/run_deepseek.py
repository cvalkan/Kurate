...
SUMMARY_KEY = "deepseek:deepseek-v4-pro"
PARALLEL = 15
...
async def run():
    ...
    total = len(paper_ids)
    print(f"Generating DeepSeek summaries for {total} papers ({PARALLEL}x parallel)", flush=True)
        ...
        async with sem:
            ...
            try:
                ...
                if result and result.get("summary") and len(result["summary"]) > 50:
                    ...
                    if ratings:
                        update["ai_ratings_by_model.deepseek"] = ratings
