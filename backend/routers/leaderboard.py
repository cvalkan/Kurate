...
@router.get("/share/{paper_id}")
async def get_share_data(paper_id: str):
    """Get shareable data for any paper — used by the universal share page."""
    from routers.badges import CATEGORIES as _CAT_NAMES, _get_tier

    paper = await db.papers.find_one({"id": paper_id}, {"_id": 0, "id": 1, "title": 1, "authors": 1, "categories": 1, "ai_rating": 1})
    if not paper:
    ...
    ranking = await db.rankings.find_one(
        {"paper_id": paper_id},
        {"_id": 0, "rank_ts": 1, "rank": 1, "ts_score": 1, "ts_sigma": 1, "score": 1,
         "win_rate": 1, "wins": 1, "losses": 1, "comparisons": 1, "category": 1},
    )
    ...
    ts_sigma = ranking.get("ts_sigma") if ranking else None

    # Check if this paper has a badge (top 3 in any archive by ts_score)
    tier_name = None
    archive_label = None
    badge_rank = None
    from core.auth import get_settings
    ...
    if primary_cat:
        archives = await db.leaderboard_archives.find(
            {"category": primary_cat, "leaderboard.id": paper_id,
             "period_type": archive_config.get(primary_cat, default_freq)},
            {"_id": 0, "leaderboard": 1, "label": 1},
        ).sort([("year", -1), ("week", -1), ("month", -1)]).to_list(5)

        for a in archives:
            lb = a.get("leaderboard", [])
            sorted_lb = sorted(lb, key=lambda p: (p.get("ts_score") or p.get("score") or 0), reverse=True)
            pos = next((i + 1 for i, p in enumerate(sorted_lb) if p.get("id") == paper_id), 999)
            tier = _get_tier(pos)
            if tier:
                tier_name = tier["name"]
                badge_rank = pos
    ...
    return {
        ...
        "rank": badge_rank or rank,
        "total_in_category": total,
        "score": ts_score,
        "ci": round(1.96 * ts_sigma * 10) if ts_sigma else None,
        "win_rate": round(ranking.get("win_rate", 0)) if ranking else None,
        "wins": ranking.get("wins") if ranking else None,
        "losses": ranking.get("losses") if ranking else None,
        "comparisons": ranking.get("comparisons") if ranking else None,
        "rating": rating_score,
        "category": primary_cat,
        "category_name": _CAT_NAMES.get(primary_cat, primary_cat) if primary_cat else None,
        "tier": tier_name,
