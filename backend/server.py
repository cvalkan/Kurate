...
AVAILABLE_MODELS = {
    ...
    "anthropic": {
        "claude-4-sonnet-20250514": "Claude 4 Sonnet (Recommended)",
...
async def compare_papers_llm(paper1: Dict, paper2: Dict, deep_analysis: bool = False, llm_model: Dict[str, str] = None, prompt_config: Dict = None) -> Dict:
...
async def run_ucb_tournament(tournament_id: str, papers: List[Dict], paper_lookup: Dict,
    ...
    while total_comparisons < max_total:
        ...
        if failed_total > 0:
            status_msg += f" ({failed_total} failed)"
        
        # DON'T write to main tournament collection during execution
        ...
        # Keep last 20 matches for live log display - only successful ones
        existing_recent = tournament_progress_cache.get(tournament_id, {}).get("recent_matches", [])
        successful_matches = [m for m in new_matches if m.get('completed') and not m.get('failed') and m.get('winner_id')]
