...
- **Bounded reconciliation (one-time + on-demand heavy scan)**: the hourly self-heal no longer runs a full O(N) rebuild. New `reconcile_check()` (scheduler ~1h) does a **cheap drift check** (Σ daily._total.matches vs `count_documents(completed,active)`, ~sub-second) and only kicks the full `_run_backfill()` when drift > max(50, 0.5%); otherwise it just refreshes the reconciled status. Manual "Rebuild" button still forces a full rebuild. Verified inline: no-drift path 0.16s (no rebuild); injected -100k drift → auto-detected → full rebuild restored exactly. Keeps steady-state cost flat as `matches` grows.
