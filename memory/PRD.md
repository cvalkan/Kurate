...
- **Prod-crash fix (post-deploy)**: first prod attempt failed silently (status frozen). Cause: `oldest["_id"].generation_time` + ObjectId-`_id` windowing (crashes/undercounts on non-ObjectId `_id`) and `{"$toDate": 0}` (int→date unsupported by Mongo optimizer). Fix: removed `_id` windowing → SINGLE full server-side aggregation (proven read-only on prod = 567,651); never-error nested `$convert` with string epoch `"1970-01-01T00:00:00Z"` fallback; `_run_backfill` now SURFACES failures into `backfill_status.error` (visible via stats-overview) and `$unset`s it on success. Re-verified read-only on prod (matches 567,651; papers 28,650) + 3 regression pytests + preview reconcile. Ready to redeploy.
