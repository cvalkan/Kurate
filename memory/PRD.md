...
- **Prod-crash fix #2 (NetworkTimeout)**: 2nd prod attempt surfaced (via the new `backfill_status.error`) `NetworkTimeout: read operation timed out` — the 4-field group aggregation over 580k matches exceeds prod's ~30s socket read timeout (`maxTimeMS` is server-side, doesn't stop a client socket timeout). Fix: heavy rebuild reads (both aggregations, ground-truth count, bulk-writes) now run through a **dedicated Mongo client `_backfill_db()`** with `socketTimeoutMS=600000` (kwargs override the URI timeout). Verified on preview (reconcile + 3 pytests). Ready to redeploy.
