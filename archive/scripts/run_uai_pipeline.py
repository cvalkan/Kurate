#!/usr/bin/env python3
"""Run the full UAI pipeline: thinking summaries → pairwise matches → single-item scoring."""
...
DATASET_ID = "uai-2024"
NUM_PAIRS = 750  # ~15 matches per paper for 100 papers
    ...
    async def gen_one(paper):
        ...
        async with sem:
            try:
                ...
                print(f"  [Step 1] Error: {str(e)[:80]}", flush=True)
...
async def step2_run_pairwise_matches():
    """Run pairwise tournament matches using thinking summaries with round-robin judges."""
    ...
    # Generate cross-tier pairs (different GT scores)
    pairs = []
    ...
    async def run_one(p1_id, p2_id, idx):
        ...
        async with sem:
            ...
            try:
                ...
                print(f"  [Step 2] Match error: {str(e)[:80]}", flush=True)
...
async def step3_single_item_scoring():
    """Run single-item scoring on all UAI papers."""
    from core.config import db, EMERGENT_LLM_KEY
    ...
    async def score_one(paper):
        ...
        async with sem:
            try:
                ...
                print(f"  [Step 3] Error: {str(e)[:80]}", flush=True)
...
async def main():
    print(f"=== UAI Pipeline ({DATASET_ID}) ===", flush=True)
